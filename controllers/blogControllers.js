...
let postRatings = asyncErrorHandler(async (req, res) => {
...
let getRating = asyncErrorHandler(async (req, res) => {
    let blogId = req.params.id
    const ratings = await Ratings.find({ blogId: blogId })
    res.status(200).json({
        status: "success",
        blogId,
        data: {
...
module.exports = {
    postBlog, getBlog, getBlogs, updateBlog, deleteBlog, postRatings, getRating
}